...
heatmap(t(matrix(unlist(train[best[1],-65]), nrow=8)),
        Colv = NA, Rowv = NA,
        col=c("white","black" ))

heatmap(t(matrix(unlist(train[best[2],-65]), nrow=8)), 
        Colv = NA, Rowv = NA,
        col=c("white" ,"black"))

#worst cases visualization
#Reshape features for each of these cases as matrix 8x8
# and visualize the corresponding digits

heatmap(t(matrix(unlist(train[Worse[1],-65]), nrow=8)),
        Colv = NA, Rowv = NA,col=c("white","black"))

heatmap(t(matrix(unlist(train[Worse[2],-65]), nrow=8)),
        Colv = NA, Rowv = NA,col=c("white","black"))

heatmap(t(matrix(unlist(train[Worse[3],-65]), nrow=8)),
        Colv = NA, Rowv = NA,col=c("white","black"))
